...
g_loadinfoview = function loadinfoview(lform = "")
{	
...
$(document).ready(function(){
	...
	$('aside#fullscreen_picture div.button-info').on('click.infoViewInfo', function() {
		...
		$('div#infocontent').toggleClass('displayinfo');
		$('aside#fullscreen_picture div.button-rightarrow').toggleClass('displayinfo');
		
		g_loadinfoview();
	});

	$('div#infocontent h4.button-exif').on('click.exifInfo', function() {
		
		$('div#infocontent h3#exif').toggle();
		if($('div#infocontent h3#exif').is(':visible')) {
			$('div#infocontent h4.button-exif div span.material-symbols-outlined').html("collapse_all");  }
		else {
			$('div#infocontent h4.button-exif div span.material-symbols-outlined').html("expand_all");}
	});
	
	$('div#infocontent h4.edit_ux').find('button.edit, button.cancel').on('click.infoViewEdit', function() {
				
		$(this).parent().children().toggle();
		
		let data=$(this).parent().attr('data-form');
		
		$('h3.ux-'+data+' input, h3.ux-'+data+' select, h3.ux-'+data+' span').toggle();
	});	
});